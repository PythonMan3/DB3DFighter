...
class Model:
    ...
    def animate_node(self, world):
        ...
        # アニメーション切り替え補間
        if self.smoother_weight > 0.0:
            for node in self.nodes:
                a = node.combined_matrix
                b = node.smoother_matrix
                a = a * (1.0 - self.smoother_weight) + b * self.smoother_weight
                node.combined_matrix = a
                node.skinning_matrix = a * node.offset_matrix
    ...
    def save_smoother(self):
        for node in self.nodes:
            node.smoother_matrix = node.combined_matrix
...
if __name__ == "__main__":
    model = Model()
    model.import_file("data/DB/goku-ssj_BouncingFightIdle.fbx")
    #show_skeleton_tree(model.nodes[0])
    
    root = model.find_node('mixamorig:Hips')
    #for anim in model.animations:
    #    for key in model.animations[0].keys:
    #        print(key.node_name)
    print(root.name)
    ...
    for key in model.animations[0].keys:
        print(key.node_name)
        print(key.position_times)
        print(key.positions)
        print(key.rotation_times)
        print(key.rotations)
        print(key.scale_times)
        print(key.scales)
        print()
    """
    """
