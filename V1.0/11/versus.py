...
class DB3DFighterVersus:
    ...
    def init(self):
        ...
        # タイトル初期化
        self.title = title.GameTitle(self.width, self.height)
    ...
    def move(self):
        if self.state == GAME_STATE.STATE_TITLE:
            self.move_title()
        if self.state == GAME_STATE.STATE_START_FIGHT:
    ...
    def move_fighting(self):
        ...
        for i in range(2):
            self.gauge[i].move()
            if self.gauge[i].value <= 0 and self.gauge[i].prev_value <= 0:
                #self.gauge[1-i].win_count += 1
                #self.gauge[i].reset()
                pass
    ...
    def draw(self, mdl_rot):
        if self.state == GAME_STATE.STATE_TITLE:
            ...
            self.draw_start_fight(mdl_rot)
        elif self.state == GAME_STATE.STAGE_FIGHTING:
            self.draw_fighting(mdl_rot)
        elif self.state == GAME_STATE.STATE_FINISH_FIGHT:
            self.draw_finish_fight(mdl_rot)
        elif self.state == GAME_STATE.STATE_WINNER:
            self.draw_winner(mdl_rot)
        
    def draw_title(self):
        self.title.draw(self.system_time)
        
    def draw_winner(self, mdl_rot):
        self.draw_fighting(mdl_rot)
        if not self.winner_renderer:
            ...
            y = 400.0
            self.winner_renderer = image_renderer.image_renderer(x, y, width, height, "../src/data/image/Winner.png", self.width, self.height)
        self.winner_renderer.draw()
        
    def draw_finish_fight(self, mdl_rot):
        self.draw_fighting(mdl_rot)
        text = "K.O."
    ...
    def draw_start_fight(self, mdl_rot):
        self.draw_fighting(mdl_rot)
    ...
    def draw_fighting(self, mdl_rot):
        # 射影行列
