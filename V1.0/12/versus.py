...
class DB3DFighterVersus:
    ...
    def init_players(self, player_indexs):
        ...
        for i in range(2):
            self.player[i] = CPlayer(self, self.window, i, model.MODEL_FILE_NAME, players[player_indexs[i]], scales[player_indexs[i]])
        for i in range(2):
    ...
    def init(self):
        ...
        # タイトル初期化
        self.title = title.GameTitle(self.window, self.width, self.height)
        # キャラ選択初期化
        self.select_character = select_character.SelectChara(self.window, self.width, self.height)
